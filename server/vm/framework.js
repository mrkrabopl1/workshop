...
const wrapFunction = (key, fn) => {
  ...
  const cloneInterface = (anInterface) => {
    ...
    for (const key in anInterface) {
      ...
      clone[key+1] = wrapFunction(key, fn);
    }
    return clone;
  };

  const cloneVM = cloneInterface(vm)
